...
export function simplifySnak(snak, options = {}) {
    ...
    if (keepTypes || keepSnaktypes || keepHashes) {
        ...
        if (keepTypes && 'datatype' in snak)
            valueObj.datatype = snak.datatype;
        if (keepSnaktypes)
...
function parseKeepOptions(options = {}) {
    if (options.keepAll) {
...
function timeSnakPrecisionIsTooLow(snak, minTimePrecision) {
    if (minTimePrecision == null)
        return false;
    if (!('datatype' in snak) || snak.datatype !== 'time' || snak.snaktype !== 'value')
        return false;
