...
const simplifyClaim = function (claim, ...options) {
  ...
  var datatype, datavalue, isQualifierSnak, isReferenceSnak, isSomeOrNoValue
  if (mainsnak) {
  ...
  const value = isSomeOrNoValue ? datavalue : parseClaim(datatype, datavalue, options, claim.id)
