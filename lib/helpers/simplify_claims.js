...
const simplifyClaim = function (claim, ...options) {
  ...
  var { keepQualifiers, keepReferences, keepIds, keepHashes, keepTypes, keepSnaktypes, keepRanks, keepAll } = options
  if (keepAll) {
    keepQualifiers = keepReferences = keepIds = keepHashes = keepTypes = keepSnaktypes = keepRanks = true
  }
