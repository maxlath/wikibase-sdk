...
helpers.isGuid = guid => /^(Q|P|L)\d+(-(F|S)\d+)?\$[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(guid)
...
helpers.getNumericId = id => {
  if (!(helpers.isEntityId(id))) throw new Error(`invalid entity id: ${id}`)
